FourTaStiC_dist <- function(data, L, E ,Time = NULL, E.default = TRUE, pp = 0.08, C = 0, alpha=NULL){
  if (missing(data))
  ...
  if (!is.numeric(E) || length(E) < 1) {
    stop("Argument 'E' must be a numeric value or numeric vector.")
  }
  if (!is.numeric(C) || length(C) != 1 || C < 0) {
    stop("`C` must be a single numeric value greater than or equal to zero.")
  }
